...
async def notify_album_owner(context: ContextTypes.DEFAULT_TYPE, owner_id: int, visitor, album_name: str):
    """通知相册创建者有人访问了相册"""
    try:
        # 构建访问者信息
        visitor_info = []
        ...
        visitor_display = " ".join(visitor_info) if visitor_info else f"用户 {visitor.id}"
        ...
        logger.info(f"已发送访问通知给相册创建者 {owner_id}: {visitor_display} 访问了 {album_name}")
        ...
        if "bot can't initiate conversation" in error_msg.lower() or "chat not found" in error_msg.lower():
            logger.warning(f"无法通知用户 {owner_id}: 该用户需要先给机器人发送 /start 启动对话")
...
async def require_channel_membership(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if not is_member:
        try:
            ...
            channel_link = chat.invite_link or f"https://t.me/c/{str(config.PUBLIC_CHANNEL_ID)[4:]}"
        except:
            channel_title = "公开频道"
            channel_link = f"https://t.me/c/{str(config.PUBLIC_CHANNEL_ID)[4:]}"

        keyboard = [[InlineKeyboardButton(f"👉 订阅 {channel_title}", url=channel_link)]]
...
async def show_batch_album_selection(update: Update, context: ContextTypes.DEFAULT_TYPE, count: int):
    """显示批量相册选择列表"""
    user = update.effective_user
    ...
    # 新增相册选项
    keyboard.append([InlineKeyboardButton("➕ 创建新相册", callback_data="batch_create_album")])

    # 跳过选择（存默认相册）
    keyboard.append([InlineKeyboardButton("🔘 跳过（存默认相册）", callback_data="batch_select_default")])
...
async def show_album_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 新增相册选项
    keyboard.append([InlineKeyboardButton("➕ 创建新相册", callback_data="select_album_new")])

    # 跳过选择（存默认相册）
    keyboard.append([InlineKeyboardButton("🔘 跳过（存默认相册）", callback_data="select_album_default")])
...
点击下方按钮订阅，然后再回来使用机器人 👇"""
    ...
    # ========== 权限设置 ==========
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("« 取消", callback_data=f"settings_album_{album_id}")
            ]])
        )
...
async def process_batch_save(update: Update, context: ContextTypes.DEFAULT_TYPE, album_id: int):
    """批量保存媒体到相册"""
    query = update.callback_query
    user = update.effective_user
    ...
    # 显示批量公开/保存选择
    keyboard = [
        [InlineKeyboardButton("📢 全部公开到频道（需审核）", callback_data="batch_publish_public")],
...
async def publish_batch_media(update: Update, context: ContextTypes.DEFAULT_TYPE, is_public: bool):
    """批量发布媒体"""
    query = update.callback_query
    user = update.effective_user
...
⏳ 等待管理员审核..."""
                ...
                review_keyboard = InlineKeyboardMarkup([
                    [
                        InlineKeyboardButton("✅ 同意", callback_data=f"review_approve_{review_id}"),
...
async def ask_public_or_private(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("📢 公开到频道（需审核）", callback_data="publish_public")],
...
async def publish_media(update: Update, context: ContextTypes.DEFAULT_TYPE, is_public: bool):
    """发布媒体"""
    query = update.callback_query
    user = update.effective_user
    ...
    album_id = context.user_data.get('selected_album_id') or db.get_default_album(user.id)
...
async def show_albums_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard.append([InlineKeyboardButton("➕ 创建新相册", callback_data="menu_create")])
...
async def show_album_details(update: Update, context: ContextTypes.DEFAULT_TYPE, album_id: int):
    """显示相册详情 - 带媒体预览"""
    query = update.callback_query
    user = update.effective_user
...
⏰ 有效期: {f"{album['expiry_hours']}小时" if album['expiry_hours'] > 0 else '永久'}"""
        ...
        if total > 1:
            nav_buttons.append(InlineKeyboardButton("▶️ 下一张", callback_data=f"preview_next_{album_id}_1"))

        if nav_buttons:
            keyboard.append(nav_buttons)

        keyboard.extend([
            [InlineKeyboardButton("🗑️ 删除此媒体", callback_data=f"delete_media_{album_id}_{current_media['media_id']}_{current_idx}")],
            [InlineKeyboardButton("🔗 分享相册", callback_data=f"share_album_{album_id}")],
            [InlineKeyboardButton("📋 访问日志", callback_data=f"access_album_{album_id}")],
            [InlineKeyboardButton("⚙️ 权限设置", callback_data=f"settings_album_{album_id}")],
            [InlineKeyboardButton("✏️ 重命名", callback_data=f"rename_album_{album_id}")],
            [InlineKeyboardButton("🗑️ 删除相册", callback_data=f"delete_album_{album_id}")],
...
async def show_preview(update: Update, context: ContextTypes.DEFAULT_TYPE, album_id: int, idx: int, is_owner: bool = False):
...
⏰ 有效期: {f"{album['expiry_hours']}小时" if album['expiry_hours'] > 0 else '永久'}"""
        ...
        protect_text = "🔒 此内容受保护，无法转发或保存" if allow_download == 0 else "💾 允许下载和转发"
        caption = f"""📂 {album['name']} ({idx + 1}/{total})
...
{protect_text}"""
    ...
    if idx > 0:
        if is_owner:
            nav_buttons.append(InlineKeyboardButton("◀️ 上一张", callback_data=f"preview_prev_{album_id}_{idx-1}"))
        else:
            nav_buttons.append(InlineKeyboardButton("◀️ 上一张", callback_data=f"shared_prev_{album_id}_{idx-1}"))

    if idx < total - 1:
        if is_owner:
            nav_buttons.append(InlineKeyboardButton("▶️ 下一张", callback_data=f"preview_next_{album_id}_{idx+1}"))
        else:
            nav_buttons.append(InlineKeyboardButton("▶️ 下一张", callback_data=f"shared_next_{album_id}_{idx+1}"))
    ...
    if is_owner:
        keyboard.extend([
            [InlineKeyboardButton("🗑️ 删除此媒体", callback_data=f"delete_media_{album_id}_{current_media['media_id']}_{idx}")],
            [InlineKeyboardButton("🔗 分享相册", callback_data=f"share_album_{album_id}")],
            [InlineKeyboardButton("📋 访问日志", callback_data=f"access_album_{album_id}")],
            [InlineKeyboardButton("⚙️ 权限设置", callback_data=f"settings_album_{album_id}")],
            [InlineKeyboardButton("« 返回相册列表", callback_data="menu_albums")]
        ...
        # 访客显示创建自己相册的按钮
        keyboard.append([InlineKeyboardButton("🤖 使用机器人创建相册", url=f"https://t.me/{context.bot.username}")])
    ...
    try:
        ...
        # 获取自动删除设置（仅对访客有效）
        auto_delete_seconds = album.get('auto_delete_seconds', 600) if not is_owner else 0
...
async def handle_waiting_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if waiting_for == 'album_name':
        ...
        keyboard = [
            [InlineKeyboardButton("📂 查看相册", callback_data=f"view_album_{album_id}")],
            [InlineKeyboardButton("🔗 分享相册", callback_data=f"share_album_{album_id}")],
            [InlineKeyboardButton("« 返回相册列表", callback_data="menu_albums")]
        ]
    ...
    # ========== 设置自动删除时间 ==========
        ...
        try:
            ...
            context.user_data.pop('waiting_for', None)
            await update.message.reply_text(
                f"✅ 自动删除时间已设置为: {seconds}秒" if seconds > 0 else "✅ 已关闭自动删除",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« 返回设置", callback_data=f"settings_album_{album_id}")]])
            )
        except ValueError:
    ...
    # ========== 管理员功能 ==========
        ...
        try:
            ...
            with open('config.py', 'r') as f:
                content = f.read()
            content = re.sub(r"PUBLIC_CHANNEL_ID = -?\d+", f"PUBLIC_CHANNEL_ID = {new_channel_id}", content)
            ...
            keyboard = [[InlineKeyboardButton("« 返回系统设置", callback_data="admin_settings")]]
            await update.message.reply_text(
                f"✅ 公开频道ID已更新为: {new_channel_id}\n\n注意：此更改在内存中已生效，重启后将永久保存。",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
        except ValueError:
            await update.message.reply_text("❌ 无效的频道ID，请输入数字格式（如: -1001234567890）")
        ...
        try:
            ...
            with open('config.py', 'r') as f:
                content = f.read()
            content = re.sub(r"PRIVATE_GROUP_ID = -?\d+", f"PRIVATE_GROUP_ID = {new_group_id}", content)
            with open('config.py', 'w') as f:
                f.write(content)
            ...
            await update.message.reply_text("❌ 无效的群组ID，请输入数字格式（如: -1001234567890）")
...
async def start_rename_album(update: Update, context: ContextTypes.DEFAULT_TYPE, album_id: int):
    """开始重命名相册"""
    query = update.callback_query
...
async def confirm_delete_album(update: Update, context: ContextTypes.DEFAULT_TYPE, album_id: int):
    """确认删除相册"""
    query = update.callback_query
...
async def execute_delete_album(update: Update, context: ContextTypes.DEFAULT_TYPE, album_id: int):
    """执行删除相册"""
    query = update.callback_query
    user = update.effective_user
...
async def confirm_delete_media(update: Update, context: ContextTypes.DEFAULT_TYPE, album_id: int, media_id: int, idx: int):
    """确认删除单个媒体"""
    query = update.callback_query
    user = update.effective_user
    ...
    keyboard = [
        [
            InlineKeyboardButton("✅ 确认删除", callback_data=f"confirm_del_media_{album_id}_{media_id}_{idx}"),
            InlineKeyboardButton("❌ 取消", callback_data=f"preview_next_{album_id}_{idx}")
        ]
    ]
...
async def execute_delete_media(update: Update, context: ContextTypes.DEFAULT_TYPE, album_id: int, media_id: int):
    """执行删除单个媒体"""
    query = update.callback_query
    user = update.effective_user
...
async def show_share_options(update: Update, context: ContextTypes.DEFAULT_TYPE, album_id: int):
    """显示分享选项"""
    query = update.callback_query
    user = update.effective_user
...
async def show_access_logs(update: Update, context: ContextTypes.DEFAULT_TYPE, album_id: int):
    """显示访问日志"""
    query = update.callback_query
    user = update.effective_user
...
🚫 黑名单人数: {len(blacklist)}\n"""
    ...
    keyboard.append([InlineKeyboardButton("« 返回相册详情", callback_data=f"view_album_{album_id}")])
...
async def show_album_settings(update: Update, context: ContextTypes.DEFAULT_TYPE, album_id: int):
    """显示相册权限设置"""
    query = update.callback_query
    user = update.effective_user
...
点击下方按钮修改设置:"""
    ...
    keyboard = [
        [InlineKeyboardButton("👥 设置人数限制", callback_data=f"set_limit_{album_id}_viewers")],
        [InlineKeyboardButton("⏰ 设置有效期", callback_data=f"set_limit_{album_id}_expiry")],
        [InlineKeyboardButton("🕐 设置自动删除", callback_data=f"set_auto_delete_{album_id}")],
        [InlineKeyboardButton("🔒 内容保护", callback_data=f"toggle_protect_{album_id}")],
        [InlineKeyboardButton("💾 下载权限", callback_data=f"toggle_download_{album_id}")],
        [InlineKeyboardButton("🚫 解除所有限制", callback_data=f"set_limit_{album_id}_none")],
        [InlineKeyboardButton("« 返回相册详情", callback_data=f"view_album_{album_id}")]
    ]
...
async def start_set_limit(update: Update, context: ContextTypes.DEFAULT_TYPE, album_id: int, limit_type: str):
    """开始设置限制"""
    query = update.callback_query
    ...
    if limit_type == 'viewers':
        ...
        current = f"{album['expiry_hours']}小时" if album['expiry_hours'] > 0 else '永久'
...
async def block_user_from_album(update: Update, context: ContextTypes.DEFAULT_TYPE, album_id: int, blocked_user_id: int):
    """将用户加入黑名单"""
    query = update.callback_query
    user = update.effective_user
...
async def view_shared_album(update: Update, context: ContextTypes.DEFAULT_TYPE, album_id: int, token: Optional[str] = None):
    """通过分享链接查看相册"""
    user = update.effective_user
    ...
    if album['max_viewers'] > 0:
        viewer_count = db.get_unique_viewers_count(album_id)
        if viewer_count >= album['max_viewers'] and not db.has_user_viewed(album_id, user.id):
...
🔒 此内容受保护，无法转发或保存"""
    ...
    if total > 1:
        nav_buttons.append(InlineKeyboardButton("▶️ 下一张", callback_data=f"shared_next_{album_id}_1"))
...
async def approve_review(update: Update, context: ContextTypes.DEFAULT_TYPE, review_id: int):
    """管理员审核通过"""
    query = update.callback_query
...
🤖 <a href="https://t.me/{context.bot.username}">使用机器人创建</a>"""
        ...
        try:
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("« 返回待审核列表", callback_data="admin_pending")
                ]])
            )
        except:
...
async def reject_review(update: Update, context: ContextTypes.DEFAULT_TYPE, review_id: int):
    """管理员审核拒绝"""
    query = update.callback_query
    admin_user = update.effective_user
...
👀 总访问次数: {stats_data['total_accesses']}"""
    ...
    # 管理员额外显示按钮
    if is_admin(user.id):
        keyboard.insert(0, [InlineKeyboardButton("👑 管理员菜单", callback_data="admin_menu")])
...
async def show_admin_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton(f"📋 待审核 ({pending_count})", callback_data="admin_pending")],
...
共 {len(reviews)} 条待审核"""
    ...
    for review in reviews[:10]:
        ...
        keyboard.append([
            InlineKeyboardButton(f"✅ 通过 #{review['review_id']}", callback_data=f"approve_review_{review['review_id']}"),
            InlineKeyboardButton(f"❌ 拒绝 #{review['review_id']}", callback_data=f"reject_review_{review['review_id']}")
        ])

    keyboard.append([InlineKeyboardButton("« 返回管理员菜单", callback_data="admin_menu")])
...
⚠️ 注意：以下操作不可恢复，请谨慎使用！"""
    ...
    keyboard = [
        [InlineKeyboardButton("🧹 清理已删除相册的媒体记录", callback_data="cleanup_orphan_media")],
